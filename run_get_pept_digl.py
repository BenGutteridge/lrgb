...
import torch
import logging
import random # not controlled by master seed I think
...
import datetime
def write_to_file(filename, string, number):
    """
    Writes a string to a text file with a number, the current date and time.
    ...
    # Open the file in append mode and write the number, timestamp, and string
    with open(filename, 'a') as f:
        f.write(str(number) + ' ' + timestamp + ' ' + string + '\n')

repeat = 3
import argparse
def parse_args(i) -> argparse.Namespace:
    ...
    extra_args = [
        'out_dir results/PEPT_DIGL_PAPER',
    ...
    for k, v in search_args.items():
        extra_args.append(' '.join([str(k), str(v)]))
