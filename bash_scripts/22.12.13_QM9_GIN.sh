  ...
  for d in "${hidden_dims[@]}" ; do
    for bs in "${batch_sizes[@]}" ; do
      ...
      for run in "${runs[@]}" ; do
        ...
        python main.py --cfg "$run" --repeat 1 model.type "${model_type}" device cuda dataset.dir datasets out_dir "$DIR" optim.max_epoch 200 train.batch_size "$bs" gnn.dim_inner "$d" gnn.layers_mp "$L"
