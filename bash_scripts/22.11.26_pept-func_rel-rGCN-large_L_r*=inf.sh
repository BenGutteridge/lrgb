#!/bin/bash
cd ..
BATCH="pept-func-rel_rGCN_large_L_r*=inf"
...
for run in "${runs[@]}" ; do
  ...
  python main.py --cfg "$run" --repeat 1 rbar -1 device cuda dataset.dir /data/beng/datasets out_dir "results/$BATCH" optim.max_epoch 300 use_edge_labels True gnn.stage_type rel_delay_gnn
