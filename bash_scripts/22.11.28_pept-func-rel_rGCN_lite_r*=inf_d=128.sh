#!/bin/bash
cd ..
BATCH="cdt1_pept-func-rel_rGCN_lite_r*=inf_d=064"
...
for run in "${runs[@]}" ; do
  ...
  python main.py --cfg "$run" --repeat 1 rbar -1 device cuda dataset.dir /data/beng/datasets out_dir "results/$BATCH" optim.max_epoch 300 use_edge_labels True gnn.stage_type rel_delay_gnn_lite gnn.dim_inner 64 train.batch_size 128
